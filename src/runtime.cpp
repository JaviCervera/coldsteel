...
static CompilerConfig InitConfig(int argc, char *argv[])
{
  ...
  XMLNode *screenWindowed = xml ? XMLChildNamed(xml, "screen_windowed", 0) : NULL;
  XMLNode *screenResizable = xml ? XMLChildNamed(xml, "screen_resizable", 0) : NULL;
  XMLNode *screenVsync = xml ? XMLChildNamed(xml, "screen_vsync", 0) : NULL;
  if (screenWidth)
  ...
  if (screenWindowed)
  {
    if (stringc(XMLText(screenWindowed)) == "true")
  ...
  if (screenResizable && stringc(XMLText(screenResizable)) == "true")
    flags |= SCREEN_RESIZABLE;
  if (screenVsync && stringc(XMLText(screenVsync)) == "true")
    flags |= SCREEN_VSYNC;
  config.screenFlags = flags;
#ifndef EMSCRIPTEN
  if (config.screenWidth == 0)
    config.screenWidth = _DesktopWidth();
  if (config.screenHeight == 0)
    config.screenHeight = _DesktopHeight();
#else
  if (config.screenWidth == 0)
    config.screenWidth = 640;
  if (config.screenHeight == 0)
