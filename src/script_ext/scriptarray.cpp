...
void RegisterScriptArray(asIScriptEngine *engine, bool defaultArray)
{
	...
	r = engine->RegisterObjectMethod("array<T>", "void Insert(int index, const T&in value)", asFUNCTION(ScriptArrayInsertAt_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void Insert(int index, const array<T>& arr)", asFUNCTION(ScriptArrayInsertAtArray_Generic), asCALL_GENERIC); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void Add(const T&in value)", asFUNCTION(ScriptArrayInsertLast_Generic), asCALL_GENERIC); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void Remove(int index)", asFUNCTION(ScriptArrayRemoveAt_Generic), asCALL_GENERIC); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("array<T>", "int Len() const", asFUNCTION(ScriptArrayLength_Generic), asCALL_GENERIC); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("array<T>", "bool Empty() const", asFUNCTION(ScriptArrayIsEmpty_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterFuncdef("bool array<T>::Less(const T&in a, const T&in b)");
	r = engine->RegisterObjectMethod("array<T>", "void Sort(const Less &in, int startAt = 0, int count = -1)", asFUNCTION(ScriptArraySortCallback_Generic), asCALL_GENERIC); assert(r >= 0);
