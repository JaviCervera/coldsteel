...
EXPORT int CALL MaterialFilterMode(SMaterial* material) {
    if (material->getFlag(EMF_ANISOTROPIC_FILTER)) return FILTER_ANISOTROPIC;
    if (material->getFlag(EMF_TRILINEAR_FILTER)) return FILTER_TRILINEAR;
    if (material->getFlag(EMF_BILINEAR_FILTER)) return FILTER_BILINEAR;
    else return FILTER_DISABLED;
}
...
EXPORT int CALL MaterialLightingMode(SMaterial* material) {
    if (!material->getFlag(EMF_LIGHTING)) return LIGHTING_DISABLED;
    else if (material->getFlag(EMF_GOURAUD_SHADING)) return LIGHTING_GOURAUD;
    else return LIGHTING_FLAT;
}
