...
class HaxeWrapperBuilder {
  ...
  private static function getFunctions(element: Xml): Iterable<Function> {
    final functions = XmlParser.elements(element, 'cdecl')
      .filter(c -> XmlParser.attribute(c, 'kind') == 'function')
      .map(f -> Function.fromXml(f));
...
private class Constant {
  ...
  public static function fromXml(element: Xml): Constant {
    return new Constant(
      ...
      XmlParser.attribute(element, 'value'));
  }
...
private class Function {
  ...
  public static function fromXml(element: Xml): Function {
    ...
    return new Function(
      XmlParser.attribute(element, 'name'),
      XmlParser.type(XmlParser.attribute(element, 'type')),
